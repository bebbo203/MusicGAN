...
def train(g, d, loader, opt_g, opt_d, epoch_n):
    ...
    for i, batch in tqdm(enumerate(loader), desc="Epoch "+str(epoch_n)+": ", total=(len(loader) // N_SAMPLES_PER_SONG + 1)):
        ...
        errD_real = -torch.mean(output) # maximize the outputs of real samples
        errD_real.backward()
        ...
        errD_fake = torch.mean(output) # minimize the outputs of fake samples
        ...
        gradient_penalty = GAMMA * compute_gradient_penalty(d, batch.data, fake.data)
        ...
        errG = -torch.mean(output) # maximize the outputs of fake samples
        errG.backward()
        opt_g.step()
...
for epoch in range(EPOCHS):
    epoch += last_epoch + 1
    avg_err_D, avg_err_G, avg_D_real, avg_D_fake = train(g, d, train_loader, optimizer_g, optimizer_d, epoch)
